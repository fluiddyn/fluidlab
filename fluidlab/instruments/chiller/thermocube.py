...
ControlParameter_SP = ThermocubeControlParameter(description="Chiller set point 1 temperature",
                                                 value=0b00001,
                                                 nbytes=2)
ControlParameter_FT = ThermocubeControlParameter(description="Current fluid temperature at chiller coolant output",
                                                 value=0b01001,
                                                 nbytes=2)
ControlParameter_FC = ThermocubeControlParameter(description="Faults from chiller (fan, pump, RTD failure, etc.)",
                                                 value=0b01000,
                                                 nbytes=1)

ControlParameter_NO = ThermocubeControlParameter(description="No-op",
                                                 value=0,
                                                 nbytes=0)

ControlParameters = [ControlParameter_SP, ControlParameter_FT, ControlParameter_FC, ControlParameter_NO]

def thermocube_message(remote_control=True, on_off=True, dir_remote_to_chiller=True,
                       control_parameter=0b00001, data=None):
    ...
    command_byte = bytes([remote_control << 7 | on_off << 6 | dir_remote_to_chiller << 5 | cp.value & 0x0F])
    if data:
...
class ThermocubeValue(Value):
    ...
    def __init__(self, name, control_parameter=ControlParameter_NO):
        super(ThermocubeValue, self).__init__(name, doc=control_parameter.description, 
                                              command_set=None, command_get=None, check_instrument_value=False,
                                              pause_instrument=False, channel_argument=False)
        self.cp = control_parameter

    def get(self):
        self._interface.write(thermocube_message(control_parameter=self.cp, dir_remote_to_chiller=False))
        sleep(1)
    ...
    def set(self, v):
        ...
        msg = thermocube_message(control_parameter=ControlParameter_SP, dir_remote_to_chiller=True, data=data)
...
features = [
    ThermocubeValue('setpoint',
                    control_parameter=ControlParameter_SP),
    ThermocubeValue('temperature',
                    control_parameter=ControlParameter_FT),
    ThermocubeValue('fault',
                    control_parameter=ControlParameter_FC),
    ]
