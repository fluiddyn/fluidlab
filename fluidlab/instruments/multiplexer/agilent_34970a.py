...
class Agilent34970a(IEC60488):
    ...
    def __init__(self, interface=None):
        ...
        # If no key is defined, NPLC and Range settings are not changed
        # if self.NPLC["101"] exists, then NPLC will be set when scanning channel 101
        # if self.Range["203"] exists, then Manual range will be set when 
        #                                                        scanning channel 203
        #      Auto Range will be set to ON otherwise
        self.NPLC = dict()
        self.Range = dict()
        
    def set_range(self, channelNumber, manualRange=False, rangeValue=None):
        if (manualRange == False) and self.Range.has_key(str(channelNumber)):
    ...
    def write_vdc(self, channel, value):
        ...
        # Write DC Voltage
        self.interface.write('SOUR:VOLT ' + str(value) + ',(@' + str(channel) + ')')
