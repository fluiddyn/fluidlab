...
class ConductivityProbe(ObjectUsingBoard):
    ...
    def __init__(
        ...
        if has_to_config_board and board is not None and self.board is not False:
...
with extreme densities with the following volume ratio: """
    ...
    def calibrate(self, rhos, duration_1measure=4.0):
        ...
        try:
            print(
                "rhos computed from the voltages "
        ...
        print(
            "Warning: bug with anaconda and non-blocking show().\n"
    ...
    def save_calibration(self, rhos, voltages):
        ...
        txt.save_quantities_in_txt_file(
            self.path_calib + "/" + name_file, (rhos, voltages)
        )
    ...
    def measure_volts(
        self, duration, sample_rate=None, return_time=False, verbose=False
    ):
...
class MovingConductivityProbe(ConductivityProbe, Traverse):
    ...
    def move_measure(
        self, deltaz=100, speed=100, sample_rate=None, return_time=False
    ):
