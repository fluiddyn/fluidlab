...
class Profiles:
            ...
            if il % n_jump_by_modulo == 0:
                ...
                try:
                    ...
                    profile = self._exp.sprobe.move_measure(
                        deltaz=-deltaz, speed=speed_measurements
                    )
    ...
    def save_one(self, path_file, profile, time_now, rotation_rate=None):
        """Save one profile in a file."""
        profile = profile.reshape([1, profile.size])

        dicttosave = {"profiles": profile, "times": time_now}
    ...
    def plot(self, ind_file_profiles=-1, times_slice=None, hastosave=False):
        """Plot profiles."""
        figures = figs.Figures(hastosave=hastosave, path_save=self.path_save)
        ...
        for ifp in ind_file_profiles:
            ...
            dict_profiles = self.load_one_file(
                ind_file_profiles=ifp, times_slice=times_slice
            )
    ...
    def plot_2d(self, ind_file_profiles=-1, times_slice=None, hastosave=False):
        ...
        if CONTOUR:
            contours = ax.contourf(
                times / 60, zTaylor, profiles.transpose(), 50, cmap=plt.cm.jet
            )

            ax.contour(
                contours,
                levels=contours.levels[::1],
                colors="k",
                # origin=origin,
                hold="on",
            )
